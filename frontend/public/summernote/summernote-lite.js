...
var TooltipUI = /*#__PURE__*/function () {
  ...
  return TooltipUI_createClass(TooltipUI, [{
    ...
    value: function show() {
      ...
      $tooltip.find('.note-tooltip-content').content(title);
...
var summernote_lite_checkbox = renderer.create('<div class="checkbox"></div>', function ($node, options) {
  $node.html(['<label' + (options.id ? ' for="note-' + options.id + '"' : '') + '>', '<input role="checkbox" type="checkbox"' + (options.id ? ' id="note-' + options.id + '"' : ''), options.checked ? ' checked' : '', ' aria-checked="' + (options.checked ? 'true' : 'false') + '"/>', options.content ? options.content : '', '</label>'].join(''));
});
