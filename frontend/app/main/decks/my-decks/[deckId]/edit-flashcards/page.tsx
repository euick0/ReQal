...
interface PageProps {
    ...
    searchParams: Promise<{ search?: string; sortBy?: string; sortOrder?: string; page?: string; flashcardId?: string; autoOpen?: string }>
}

const EditFlashcardsPage = async ({ params, searchParams }: PageProps) => {
    const { deckId } = await params
    const { search, sortBy, sortOrder, page, flashcardId, autoOpen } = await searchParams

    const { data: deck, error: deckError } = await GetDeckById(deckId)
    if (deckError || !deck) notFound()

    const isConjugation = deck.name === "Conjugation"
    const resolvedSortOrder: SortOrder = sortOrder === "desc" ? "desc" : "asc"
    const resolvedPage = Math.max(1, Number(page ?? "1"))

    if (isConjugation) {
        const validSortColumns: ConjugationFlashcardSortColumn[] = ["phrase", "missing_word", "review_date"]
        const resolvedSortBy: ConjugationFlashcardSortColumn = validSortColumns.includes(sortBy as ConjugationFlashcardSortColumn)
            ? (sortBy as ConjugationFlashcardSortColumn)
            : "phrase"

        const { data: flashcards, count, error: flashcardsError } = await GetConjugationFlashcardsFiltered(
            deckId,
            search,
            resolvedSortBy,
            resolvedSortOrder,
            ...
            <div className="flex flex-col pl-20 w-full h-screen right-0 overflow-visible">
                ...
                <div className="flex flex-col gap-4 w-full h-full overflow-y-auto pb-10">
                    <FlashcardList
                        ...
                        autoOpenFlashcardId={autoOpen === "true" ? (flashcardId ?? null) : null}
                    />
                </div>
            </div>
        )
